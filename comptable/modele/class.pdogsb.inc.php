...
*/
	...
	public function getAllVisiteur(){
		...
		$user = PdoGsb::$monPdo->prepare($req);
		$user->execute();
		$ligne = $user->fetch();
		return $ligne;
	}
...
*/
	...
	// retourne les frais forfait cloturer 
	public function getLesFraisForfaitCR()
	{
		$req = "select idVisiteur, mois, nbJustificatifs, montantValide, dateModif 
		from fichefrais where idEtat = 'CR'";
		$idJeuRes = PdoGsb::$monPdo->prepare($req);
 ...
 */
	...
	public function hashMdpAllUser()
	{
		...
		{
			$mdp = password_hash($user['mdp']);
			$id = $user['id'];
			$strReq = "UPDATE `visiteur` SET `mdp`='$mdp' WHERE id = '$id'";
	...
	public function ValiderFrais($id, $mois)
	{
